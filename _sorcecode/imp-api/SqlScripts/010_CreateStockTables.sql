-- =============================================
-- 010: Create Stock Tables (stock_lot, stock_card, stock_cutting)
-- =============================================

-- 1. Stock Lot — FIFO queue per raw material
IF NOT EXISTS (SELECT * FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id WHERE s.name = 'imp' AND t.name = 'stock_lot')
BEGIN
    ...
    PRINT 'Created table imp.stock_lot';
END
GO

-- 2. Stock Card — transaction log (running balance)
IF NOT EXISTS (SELECT * FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id WHERE s.name = 'imp' AND t.name = 'stock_card')
BEGIN
    ...
    PRINT 'Created table imp.stock_card';
END
GO

-- 3. Stock Cutting — lot-to-export mapping
IF NOT EXISTS (SELECT * FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id WHERE s.name = 'imp' AND t.name = 'stock_cutting')
BEGIN
    ...
    PRINT 'Created table imp.stock_cutting';
END
GO
